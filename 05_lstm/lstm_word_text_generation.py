...
class TwoLayerLSTM(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        # Ensure x has shape (sequence_length=1, batch_size=1, input_size)
        # x comes in as (input_size), unsqueeze(0) makes it (1, input_size)
        # We need to add another dimension for batch_size=1
...
def train_lstm(model, dataloaders, dataset_sizes, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in phases:
            ...
            for input_sequence, target_sequence in dataloaders[phase]:
                ...
                # input_sequence and target_sequence should have shape (sequence_length, 1, input_size)
                # Squeeze the batch dimension if it's there, as initHidden assumes batch_size=1
                # Move to device
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    for i in range(input_sequence.size(0)):
                        ...
                        # criterion expects output of shape (batch_size, num_classes) and target of shape (batch_size)
                        # Since we are processing one step at a time, batch_size is 1
                        # output shape is now (output_size)
                ...
                # statistics
                # Normalize running_loss by the total number of steps in the batch (sequence length)
            ...
            # It's generally better practice to use validation loss for determining the best model
            # Change this to `if phase == 'val' and epoch_loss < best_loss:` for better practice
    ...
    # Change the best loss message to reflect validation loss if you change the condition above
...
# Define scheduler if you want to use it
...
def predict(model, word_dictionary, word_list, input_sentence, max_length = 20):
    ...
    # create_input_tensor gives (seq_len, 1, vocab_size), squeeze to (seq_len, vocab_size)
    ...
    for i in range(input_tensor.size(0)):
        with torch.no_grad(): # No need to calculate gradients during prediction
            # Pass one time step of the input sequence and the current hidden state
            # input_tensor[i] has shape (vocab_size)
    ...
    for i in range(len(input_sentence.split(" ")), max_length):
        with torch.no_grad(): # No need to calculate gradients during prediction
            ...
            # Create input tensor for the predicted word
            # The input for the next step is the one-hot encoding of the predicted word
            # Need to create a tensor of shape (input_size) for the single word
            # The output of create_input_tensor(word, ...) is (1, 1, vocab_size).
            # We need to squeeze it to (vocab_size) before passing it to the model.
            ...
            # Use the predicted word as the input for the next step
            # Pass the single time step input and the current hidden state
            # next_input_tensor has shape (vocab_size)
